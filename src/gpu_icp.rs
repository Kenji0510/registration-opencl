...
const KERNEL_SRC: &str = include_str!("kernels/icp.cl");
...
impl OclIcpContext {
    pub fn new(rt: OclRuntime) -> Result<Self> {
        ...
        let kernel_func = Kernel::builder()
            .program(&program)
            .name("compute_pt2pl_linear_system")
            .queue(rt.queue.clone())
            .global_work_size(1)
            .local_work_size(1)
    ...
    pub fn compute_icp(
        ...
        d_target_normals: &Buffer<f32>,
        num_target: usize,
        d_indices: &Buffer<i32>,
        d_distances: &Buffer<f32>,
        max_dist_sq: f32,
    ) -> Result<(Array2<f64>, Array1<f64>)> {
        println!("ICP: num_source = {}, num_target = {}", num_source, num_target);
...

        {
            ...
            self.kernel_func
                .set_arg(1, d_target_pts)
                .context("Failed to set d_target_pts arg")?;
            self.kernel_func
                .set_arg(2, d_target_normals)
                .context("Failed to set d_target_normals arg")?;
            self.kernel_func
                .set_arg(3, d_indices)
                .context("Failed to set d_indices arg")?;
            self.kernel_func
                .set_arg(4, d_distances)
                .context("Failed to set d_distances arg")?;
            self.kernel_func
                .set_arg(5, num_source as i32)
                .context("Failed to set num_source arg")?;
            self.kernel_func
                .set_arg(6, num_target as i32)
                .context("Failed to set num_target arg")?;
            self.kernel_func
                .set_arg(7, max_dist_sq)
