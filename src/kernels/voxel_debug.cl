...
inline void add_to_global(
    ...
    atomic_inc((volatile __global int*)dropped);
}

__kernel void init_table(__global ulong* table_keys, __global int* table_remap, int table_size, __global int* dropped) {
    int idx = get_global_id(0);
    if (idx == 0) dropped[0] = 0;
...
__kernel void insert_points(
    ...
    if (idx < num_points) {
        ...
        if (!stored) {
            add_to_global(hash_key, px, py, pz, 1, table_keys, table_centroids, table_counts, table_size, dropped);
        }
    }
    ...
    for (int i = tid; i < SHARED_TABLE_SIZE; i += ldim) {
        ...
        if (key != EMPTY_KEY) {
            ...
            add_to_global((unsigned long)key, sx, sy, sz, sc, table_keys, table_centroids, table_counts, table_size, dropped);
        }
    }
}
